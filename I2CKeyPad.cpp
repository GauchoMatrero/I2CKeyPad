...
I2CKeyPad::I2CKeyPad(const uint8_t deviceAddress)
{
...
bool I2CKeyPad::isConnected()
{
	...
	uint8_t i2code;
	i2code==TinyI2C.start(_address, 0);
	TinyI2C.stop();
	//    Serial.print("I2Code=");
	//	Serial.println(i2code);
	return i2code == 0;
}
...
// cf 22/2/23
	...
	TinyI2C.start(_address, 0); // write to PCF8574
	i2code=TinyI2C.write(mask);
	//	Serial.println();
	//	Serial.print("i2c write return code=");
	//	Serial.println(i2code);
	if (i2code == !true) // Error writting
	{	// NOK TRANSMISION
		//  set communication error
		return 0xFF;
	}
	TinyI2C.restart(_address, 1); // read 1 byte
	retvalue = TinyI2C.read();
	TinyI2C.stop();
